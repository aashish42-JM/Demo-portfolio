    ...
    <div className="h-full flex flex-col">
      <div className="flex-1 overflow-y-auto p-10 space-y-10">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="glass neon-border rounded-2xl px-8 py-5 text-center">
            <div className="font-mono text-xs text-[#4fc3f7]/70 uppercase tracking-widest mb-3">EARNED XP</div>
        ...
        <div>
          <div className="flex items-center gap-4 mb-6">
            <Target size={24} className="text-[#4fc3f7]" />
          ...
          <div className="space-y-6 mt-4">
            {active.map((mission, i) => (
              <motion.div
                ...
                className="mission-card glass p-6 rounded-2xl border border-[rgba(79,195,247,0.15)]"
              >
                <div className="flex items-start gap-5">
                  <Circle size={28} className="text-[#4fc3f7]/40 shrink-0 mt-1" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <h3 className="font-bold text-xl text-white">{mission.title}</h3>
                      <div className="flex items-center gap-3 shrink-0">
                        <Zap size={18} className="text-yellow-400" />
                        <span className="font-mono text-sm text-yellow-400">+{mission.xp_reward} XP</span>
                      </div>
                    </div>
                    <p className="text-sm text-[#64b5f6]/70 mt-3 leading-relaxed">
                      {mission.description}
                    </p>
                    <span className="inline-block mt-4 text-xs font-mono px-4 py-2 rounded-full bg-[rgba(79,195,247,0.1)] border border-[rgba(79,195,247,0.2)] text-[#90caf9]/80">
                      {mission.category}
                    </span>
                  </div>
                </div>
          ...
          <div className="space-y-6 mt-4">
            {completed.map((mission, i) => (
              <motion.div
                ...
                className="flex items-start gap-5 p-6 rounded-2xl border border-green-400/10 bg-green-400/5">
                <CheckCircle2 size={28} className="text-green-400 shrink-0 mt-1" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4">
                    <h3 className="font-bold text-xl text-green-400/80">{mission.title}</h3>
                    <div className="flex items-center gap-3 shrink-0">
                      <Zap size={18} className="text-yellow-400" />
                      <span className="font-mono text-sm text-yellow-400">{mission.xp_reward} XP</span>
                    </div>
                  </div>
                  <p className="text-sm text-white/40 mt-3">{mission.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
