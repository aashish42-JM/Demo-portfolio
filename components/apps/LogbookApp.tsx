    ...
    <div className="h-full flex flex-col">
      <div className="flex-1 overflow-y-auto p-10 space-y-8">
        <div className="flex items-center gap-5">
          <BookOpen className="text-[#4fc3f7]" size={32} />
          <div>
            <h1 className="text-3xl font-bold text-white">Developer Logbook</h1>
        ...
        >
          <div className="font-mono text-sm text-[#4fc3f7]/70 mb-5 uppercase tracking-widest">
            // Next Chapter
          </div>
          <div className="flex flex-wrap gap-4">
            {["Master React", "Deploy with Vercel", "Learn TypeScript deeply", "Build ML model", "Open source contribution"].map(
              (goal) => (
                <span
                  key={goal}
                  className="text-sm font-mono px-5 py-3 rounded-full bg-[rgba(79,195,247,0.15)] border border-[rgba(79,195,247,0.25)] text-[#90caf9]"
                >
                  → {goal}
                </span>
              )
        ...
        {LOGBOOK_ENTRIES.map((entry, index) => (
          ...
          >
            <div className="flex items-start justify-between gap-5 mb-4">
              <h3 className="font-bold text-xl text-white">{entry.title}</h3>
              <div className="flex items-center gap-3 shrink-0 text-[#64b5f6]/60">
                <Calendar size={16} />
                <span className="font-mono text-xs">{entry.week}</span>
              </div>
            </div>

            <p className="text-sm text-[#64b5f6]/80 leading-relaxed mb-5">{entry.content}</p>

            <div className="flex flex-wrap gap-3">
              {entry.tags.map((tag) => (
                <span
                  key={tag}
                  className="flex items-center gap-3 text-xs font-mono px-4 py-2 rounded-full
        ...
        <div className="text-center py-8">
          ...
          <div className="flex justify-center mt-5">
            <div className="w-20 h-px bg-[rgba(79,195,247,0.3)]" />
          </div>
        </div>
      </div>
    </div>
